...
def run_once(state: Path, initial: dict[str, Any], current: dict[str, Any], confirmation: str,
             cancelled: bool, backend: Callable[[dict[str, Any]], None]) -> str:
    """Revalidate, consume once before effect, and return a truthful outcome."""
    if cancelled: return "cancelled_no_write"
    first = evaluate(initial, confirmation)
    if first["result"] != "guard_passed_no_write_performed": raise HarnessError("confirmation failed")
